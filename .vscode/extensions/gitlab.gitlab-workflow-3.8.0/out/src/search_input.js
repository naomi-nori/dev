...
function getSearchInput(description) {
...
function showSearchInputFor(noteableType) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const query = yield getSearchInput('Search in title or description. (Check extension page for search with filters)');
        const queryString = yield parseQuery(query, noteableType);
        yield openers.openUrl(`${project.web_url}/${noteableType}${queryString}`);
    });
}
function showIssueSearchInput() {
    return __awaiter(this, void 0, void 0, function* () {
        showSearchInputFor('issues');
    });
}
function showMergeRequestSearchInput() {
    return __awaiter(this, void 0, void 0, function* () {
        showSearchInputFor('merge_requests');
    });
}
function showProjectAdvancedSearchInput() {
    return __awaiter(this, void 0, void 0, function* () {
        const workspaceFolder = yield getCurrentWorkspaceFolderOrSelectOne();
        const project = yield gitLabService.fetchCurrentProject(workspaceFolder);
        const query = yield getSearchInput('Project Advanced Search. (Check extension page for Advanced Search)');
        const queryString = yield encodeURIComponent(query);
        const instanceUrl = yield createGitService(workspaceFolder).fetchCurrentInstanceUrl();
        // Select issues tab by default for Advanced Search
        yield openers.openUrl(`${instanceUrl}/search?search=${queryString}&project_id=${project.id}&scope=issues`);
    });
}
