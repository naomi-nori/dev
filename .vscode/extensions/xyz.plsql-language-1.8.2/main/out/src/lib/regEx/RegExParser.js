...
RegExpParser.regCommentInside = `(?:\\/\\*[\\s\\S]*?\\*\\/\\s*|--.*\\s+)*\\s*`; // a bit slower !
...
RegExpParser.regBody = `(?:\\b(procedure|function)\\b\\s+(\"?${RegExpParser.REG_WORD}+\"?)[\\s\\S]*?((?:\\bas\\s+(language)\\b.[^;]*;)|;|\\b(?:is|as|begin)\\b))`;
...
RegExpParser.regJumpEnd = `(\\b(?:begin|case)\\b)|(?:(\\$?\\bend\\b)\\s*(?:\\b(if|loop|case)\\b)?)`;
